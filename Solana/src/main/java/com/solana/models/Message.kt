...
class Message {
    ...
    fun serialize(): ByteArray {
        ...
        val accountAddressesLength = encodeLength(accountKeysSize)
        ...
        for (instruction in instructions) {
            ...
            compiledInstruction.keyIndicesCount = encodeLength(keysSize)
            compiledInstruction.keyIndices = keyIndices
            compiledInstruction.dataLength = encodeLength(instruction.data.size)
            compiledInstruction.data = instruction.data
            compiledInstructions.add(compiledInstruction)
            compiledInstructionsLength += compiledInstruction.length
        }
        val instructionsLength = encodeLength(compiledInstructions.size)
        val bufferSize =
            (MessageHeader.Companion.HEADER_LENGTH + RECENT_BLOCK_HASH_LENGTH + accountAddressesLength.size
    ...
    private fun getAccountKeys(): List<AccountMeta> {
        val keysList: ArrayList<AccountMeta> = accountKeys.list
